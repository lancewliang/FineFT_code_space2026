...
- Explicit custom class typing over defensive probing: Always explicitly declare custom class types on function/method parameters (e.g. `trainer: Weighted_Contexts_DQN`, `buffer: Multi_step_ReplayBuffer_multi_info`). Never leave custom class objects as untyped bare parameters that invite defensive `getattr`/`hasattr` probing.
...
- Mandatory custom class parameter typing: Whenever a function or method receives an instance of a project/domain custom class (e.g. `trainer: Weighted_Contexts_DQN`, `buffer: Multi_step_ReplayBuffer_multi_info`, `manifest: PretrainDiagnosticsManifest`, `model: ensemble_Qnet`), you MUST explicitly declare the concrete custom class type in the `def` signature. Never pass or accept custom domain objects as bare untyped arguments. Use `from __future__ import annotations` and `if TYPE_CHECKING:` to resolve circular dependencies without runtime overhead.
