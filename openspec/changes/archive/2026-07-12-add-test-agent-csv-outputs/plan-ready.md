...
- 影响规格：`openspec/changes/add-test-agent-csv-outputs/specs/fineft-low-level-test-results/spec.md`
- 影响任务：`tasks.md` 新增 `1.7 Change aggregate and detail CSV headers to English/Chinese bilingual names while preserving row values and JSON array cells.`

### 2026-07-12: CSV 表头语义改为当前中文表头
- 原因：用户确认当前实现中的中文-only 表头才是正确含义，不应要求 `英文/中文` 双语格式。
- 影响规格：`openspec/changes/add-test-agent-csv-outputs/specs/fineft-low-level-test-results/spec.md`
- 影响任务：`tasks.md` 新增 `1.8 Align CSV header tests and close verification with the current Chinese semantic headers.`
...
- 目标：让 `analysis_result.csv` 和 `trading_action_detail_epoch_<epoch_num>.csv` 的表头使用 `英文/中文` 格式，同时保持 JSON 数组单元格、数值内容和行粒度不变。
- 改动文件：`FineFT/RL/DiHFT/low_level/test_agent_index.py`、`FineFT/tests/rl/test_test_agent_index.py`
- 验证方式：运行 low-level test-agent focused tests，确认汇总 CSV 表头为双语且 JSON 数组仍可解析，明细 CSV 必要上下文和执行指标表头为双语。
...
- 目标：保留当前实现中的中文语义表头，更新测试断言和 close 验证预期，不再要求 `英文/中文` 双语格式。
- 改动文件：`FineFT/tests/rl/test_test_agent_index.py`、`openspec/changes/add-test-agent-csv-outputs/close-issues.md`
- 验证方式：运行 focused pytest，确认汇总 CSV 表头为 `标签, 初始动作, 分箱索引, 数据文件, 奖励总和, 数据长度, 换手率`，明细 CSV 表头使用当前中文语义名。
