...
- **AND** `trading_info` SHALL 包含 3 个无量纲特征：`position_exposure`、`single_holding_return_rate` 和 `single_holding_max_drawdown`
...
系统 SHALL 在低层 Q 网络 (`Qnet` / `ensemble_Qnet`) 中增加独立线性层 `fc_trading` 编码 3 维 `trading_info`。

#### Scenario: Qnet 和 ensemble_Qnet 接收 trading_info 输入
- **WHEN** 调用 `Qnet.forward()` 或 `ensemble_Qnet.forward()`
- **THEN** 必须显式传入 `trading_info` (shape `(batch, 3)`)
...
#### Scenario: Replay Buffer sample 包含 3 维 trading_info
- **WHEN** 从 Replay Buffer 执行 `sample()` 或 `sample_evaluate()`
- **THEN** 返回的 `infos["trading_info"]` 与 `next_infos["trading_info"]` 的 shape 最后一维 SHALL 等于 3
