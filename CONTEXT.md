...
**轨迹优先多步累加 (Trajectory-first N-Step Accumulation)**:
在单条连续时序轨迹上完整计算多步贴现回报后再按起始步体制路由写入经验队列的计算范式，防止跨体制非连续时序回报计算被破坏。
_Avoid_: 经验池内多步累加、跨体制时序拼接

**轮次体制课程调度 (Rotating Epoch Curriculum)**:
在多样化训练不同轮次间激活特定体制经验池子集进行平衡采样，促使各轮次快照在 Q 地形上产生显著分化的调度机制。
_Avoid_: 动态经验池轮转、轮次交替抽样

**阶段周期重置衰减 (Phase-Cyclic Parameter Decay)**:
