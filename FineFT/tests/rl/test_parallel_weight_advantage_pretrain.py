...
def test_compute_epoch_schedules_phase_cyclic_decay():
    from RL.DiHFT.low_level import parallel_diverse_train as pdt
    import pytest

    # x = 3, num_epoch = 18:
    # Epoch 0-2 (Phase 0): max -> min
    # Epoch 3-5 (Phase 1): reset max -> min
    # Epoch 6-8 (Phase 2): reset max -> min
    # Epoch 9-17: clamped to min
    p0_start = pdt.compute_epoch_training_params(
        epoch_index=0, num_epoch=18, epsilon_init=1.0, epsilon_min=0.1,
        ada_init=256.0, ada_min=0.0, lr_init=0.005, lr_min=0.001, curriculum_block_epochs=3,
    )
    p0_mid = pdt.compute_epoch_training_params(
        epoch_index=1, num_epoch=18, epsilon_init=1.0, epsilon_min=0.1,
        ada_init=256.0, ada_min=0.0, lr_init=0.005, lr_min=0.001, curriculum_block_epochs=3,
    )
    p0_end = pdt.compute_epoch_training_params(
        epoch_index=2, num_epoch=18, epsilon_init=1.0, epsilon_min=0.1,
        ada_init=256.0, ada_min=0.0, lr_init=0.005, lr_min=0.001, curriculum_block_epochs=3,
    )

    assert p0_start.epsilon == 1.0
    assert p0_start.ada == 256.0
    assert p0_mid.epsilon == pytest.approx(0.55)
    assert p0_mid.ada == pytest.approx(128.0)
    assert p0_end.epsilon == pytest.approx(0.1)
    assert p0_end.ada == pytest.approx(0.0)

    # Phase 1: reset to max at epoch 3, decay to min at epoch 5
    p1_start = pdt.compute_epoch_training_params(
        epoch_index=3, num_epoch=18, epsilon_init=1.0, epsilon_min=0.1,
        ada_init=256.0, ada_min=0.0, lr_init=0.005, lr_min=0.001, curriculum_block_epochs=3,
    )
    p1_end = pdt.compute_epoch_training_params(
        epoch_index=5, num_epoch=18, epsilon_init=1.0, epsilon_min=0.1,
        ada_init=256.0, ada_min=0.0, lr_init=0.005, lr_min=0.001, curriculum_block_epochs=3,
    )
    assert p1_start.epsilon == 1.0
    assert p1_start.ada == 256.0
    assert p1_end.epsilon == pytest.approx(0.1)
    assert p1_end.ada == pytest.approx(0.0)

    # Phase 2: reset to max at epoch 6, decay to min at epoch 8
    p2_start = pdt.compute_epoch_training_params(
        epoch_index=6, num_epoch=18, epsilon_init=1.0, epsilon_min=0.1,
        ada_init=256.0, ada_min=0.0, lr_init=0.005, lr_min=0.001, curriculum_block_epochs=3,
    )
    p2_end = pdt.compute_epoch_training_params(
        epoch_index=8, num_epoch=18, epsilon_init=1.0, epsilon_min=0.1,
        ada_init=256.0, ada_min=0.0, lr_init=0.005, lr_min=0.001, curriculum_block_epochs=3,
    )
    assert p2_start.epsilon == 1.0
    assert p2_start.ada == 256.0
    ...
    for ep in [9, 10, 15, 17]:
        ...
        assert p_late.ada == pytest.approx(0.0)
...
def test_parallel_parser_curriculum_block_epochs_default_and_flags():
    ...
    args_default = pwap.parser.parse_args([])
    assert args_default.curriculum_block_epochs == 3

    args_flag = pwap.parser.parse_args(["--curriculum_block_epochs", "5"])
    assert args_flag.curriculum_block_epochs == 5
