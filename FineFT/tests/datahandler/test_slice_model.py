...
def test_run_keeps_limit_prices_in_regular_five_label_slicing(tmp_path, monkeypatch):
    module = _load_slice_model()
    valid_dir = tmp_path / "dataset" / "10min" / "fu" / "valid"
    data_path = valid_dir / "df_fu2505.feather"
    valid_dir.mkdir(parents=True)
    pd.DataFrame(
        {
            ...
            "timestamp": pd.date_range(
                "2025-01-01 09:00:00", periods=5, freq="10min"
            ),
    ...
    class FakeWorker:
        def __init__(self, worker_data_path, *args, **kwargs):
            self.data_path = worker_data_path
            self.data_dict = {}

        def fit(self, dynamic_number, *args, **kwargs):
            assert dynamic_number == 5

        def label(self, work_dir):
            data = pd.read_feather(self.data_path)
            assert data["bid1_price"].tolist() == [100.0, 110.0, 102.0, 90.0, 103.0]
            data["label"] = [0, 4, 2, 0, 3]
            self.data_dict = {data["symbol"].iloc[0]: data}
    ...
    assert processed["bid1_price"].tolist() == [100.0, 110.0, 102.0, 90.0, 103.0]
    assert sorted(path.name for path in (valid_dir / "fu2505").iterdir()) == [
        "label_0",
        "label_1",
        "label_2",
        "label_3",
