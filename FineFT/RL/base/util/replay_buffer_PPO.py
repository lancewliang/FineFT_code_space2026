...
class ReplayBuffer:
    ...
    def store(self, s, t, p_a, a_a, a, a_logprob, r, s_, t_, p_a_, dw, done):
        self.s[self.count] = s
        self.t[self.count] = t
    ...
    def numpy_to_tensor(self):
        ...
        t = torch.tensor(self.t, dtype=torch.float).to(self.device)
        ...
        t_ = torch.tensor(self.t_, dtype=torch.float).to(self.device)
        ...
        return s, t, p_a, a_a, a, a_logprob, r, s_, t_, p_a_, dw, done
...
class ReplayBuffer_lstm:
    ...
    def reset_buffer(self):
        self.buffer = {
            ...
            "t": np.zeros([self.batch_size, self.episode_limit, 2]),
    ...
    def store_transition(self, episode_step, s, time, p_a, a_a, v, a, a_logprob, r, dw):

        self.buffer["s"][self.episode_num][episode_step] = s
        self.buffer["t"][self.episode_num][episode_step] = time
    ...
    def get_training_data(self):
        ...
        batch = {
            ...
            "t": torch.tensor(
                self.buffer["t"][:, : self.max_episode_len], dtype=torch.float32
            ),
            "p_a": torch.tensor(
