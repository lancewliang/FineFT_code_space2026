...
class DQN(object):
    def __init__(self, args):
        ...
        self.N_STATES = len(self.tech_indicator_list)
        self.ensemble_num = args.ensem_num  
        self.k_fold = args.k_fold
        self.net_list=[]
        self.optimizer_list=[]
        for _ in range(self.ensemble_num):
            constructed_net= Qnet(
            ...
            self.net_list.append(constructed_net)
            self.optimizer_list.append(torch.optim.Adam(self.net_list[-1].parameters(), lr=self.lr))
    ...
    def update(
        ...
        for i in range(len(self.net_list)):
            updated_index=i
            target_index_list=[j%len(self.net_list) for j in range(i-self.k_fold,i)]
            eval_net=self.net_list[updated_index]
            target_net_list=[self.net_list[j] for j in target_index_list]
            ...
            for i in range(len(target_net_list)):
                net=self.net_list[i]
                if i==0:
                    q_next = net(
                        state=states_,
