...
sys.path.append(".")
import re
from model.low_level import Actor_Critic_RNN
...
class PPO_discrete_RNN:
    ...
    def choose_action(self, s, info, evaluate=False):
        with torch.no_grad():
            ...
            logit = self.ac.actor(s, time_input, previous_action, avaliable_action)
            if evaluate:
    ...
    def get_value(self, s, info):
        with torch.no_grad():
            ...
            value = self.ac.critic(s, time_input, previous_action)
            return value.item()
