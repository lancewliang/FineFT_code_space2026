...
class macd_trader:
    def __init__(self, args):
        ...
        # trading parameter
        self.upper_bond = args.upper_bond
        self.lower_bond = args.lower_bond

    def act_test(self, info, previous_action):
        buy_qts = np.sum(info["bid_qyts"])
        sell_qts = np.sum(info["ask_qyts"])
        signal = (buy_qts - sell_qts) / (buy_qts + sell_qts)
        if signal > self.upper_bond:
            return max(info["avaiable_action_list"])
        if signal < self.lower_bond:
            return min(info["avaiable_action_list"])
        else:
    ...
    def test(self):
        for name, data_path in zip(
            ...
            self.test_env_instance = initiate_base_env(
                df=self.test_df,
            ...
            while True:
                true_action = self.act_test(info, previous_true_action)
                ...
                if done:
                    break
            true_action_list = np.array(true_action_list)
