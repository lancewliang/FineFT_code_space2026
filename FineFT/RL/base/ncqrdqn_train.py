...
import argparse
from model.low_level import NCQRDQN
import numpy as np
import torch
...
import re
from RL.base.util.qrdqn_util import (
    update_params,
    soft_copy_params,
...
parser.add_argument("--lr_init", type=float, default=1e-3, help="the learning rate")
...
class Risk_averse_QRDQN(object):
    def __init__(self, args):
        ...
        self.epsilon_decay = (self.epsilon_init - self.epsilon_min) / args.epsilon_step
        ...
        self.cvar_threshold = int(self.N * args.conditional_threshold)
        # loss function
        taus = (
            torch.arange(0, self.N + 1, device=self.device, dtype=torch.float32)
            / self.N
        )
    ...
    def update(
        ...
        with torch.no_grad():
            next_action_value = self.target_net(
                state=states_,
                time=time_input_,
                previous_action=previous_action_,
                avaliable_action=avaliable_action_,
            )
            next_action = pick_action_cvar(
                next_action_value, selected_range=range(self.cvar_threshold)
            )
            ...
            next_sa_quantiles = next_sa_quantiles.detach().permute(0, 2, 1)
        ...
        return (
            quantile_huber_loss,
            td_errors.detach().abs().sum(dim=1).mean(dim=1).mean(dim=0),
        )
    ...
    def act(self, state, info, epsilon):
        if np.random.uniform() > epsilon:
            ...
            action = pick_action_cvar(
                actions_value, selected_range=range(self.cvar_threshold)
            )
                ...
                if (
                    ...
                    for _ in range(self.update_times):
                        ...
                        self.writer.add_scalar(
                            tag="quantile_hubber_loss",
                            scalar_value=quantile_huber_loss,
                            global_step=self.update_counter,
                            walltime=None,
                        )
                        self.writer.add_scalar(
                            tag="td_error_sum",
                            scalar_value=td_errors,
                            global_step=self.update_counter,
                            walltime=None,
                        )
