...
from RL.util.transition_dataset import Transition_Dataset
...
parser.add_argument(
    "--result_path",
    type=str,
    default="result/DiHFT/low_leve_update_index",
...
class Weighted_Contexts_DQN:
    def __init__(self, args):
        ...
        self.train_df = os.path.join(
            self.base_path, self.dataset_name, "train", f"df_{args.df_index}.feather"
        )
        self.train_df = pd.read_feather(self.train_df)
        self.df_index = args.df_index
    ...
    def get_rank(
        ...
        with torch.no_grad():
            ...
            assert target_sa_quantiles.shape == (bs, 1, self.N)
        td_errors = target_sa_quantiles - current_sa_quantiles
        assert td_errors.shape == (bs, self.N, self.N)
        if self.if_use_hubber_loss:
            td_errors = calculate_huber_loss(td_errors)
        _, chosen_indices = find_proper_index(
            td_errors=td_errors,
            outerbond=self.outer_bond,
            reachout_index=self.reachout_index,
        )
    ...
    def train(self):
        ...
        for sample in range(self.num_sample):
            ...
            if len(epoch_reward_sum_train_list) == epoch_number:
                ...
                chosen_indices = self.evaluate_model()
                self.plot_return_curve(chosen_indices, epoch_path)
                np.save(os.path.join(epoch_path, "chosen_indices.npy"), chosen_indices)
                epoch_return_rate_train_list = []
                epoch_final_balance_train_list = []
                epoch_reward_sum_train_list = []

    def evaluate_model(self):
        buffer_evaluate = Multi_step_ReplayBuffer_multi_info(
            buffer_size=self.buffer_size,
            batch_size=self.batch_size,
            device=self.device,
            seed=self.seed,
        ...
        while True:
            a = self.act_multi_styles_pretrain(
                info, optimal_step_counter, rollout_index=1
            )
            optimal_step_counter += 1
            s_, r, done, info_ = self.test_env_instance.step(a)
            buffer_evaluate.add(s, info, a, r, s_, info_, done)
            episode_reward_sum += r
            s, info = s_, info_
            if done:
                break
        states, infos, actions, rewards, next_states, next_infos, dones = (
            buffer_evaluate.sample_evaluate()
        )
        transition_dataset = Transition_Dataset(
            states, infos, actions, rewards, next_states, next_infos, dones
        )
        transition_dataloader = DataLoader(
            transition_dataset, batch_size=self.batch_size, shuffle=False
        )
        print("optimal_step_counter", optimal_step_counter)
        chosen_indices_list = []
        for (
            states,
            infos,
            ...
            next_infos,
            dones,
        ) in transition_dataloader:
            chosen_indices = self.get_rank(
                states, infos, actions, rewards, next_states, next_infos, dones
            )
            chosen_indices_list.append(chosen_indices)
        chosen_indices = torch.cat(chosen_indices_list, dim=0)
        chosen_indices = chosen_indices.cpu().numpy()
        return chosen_indices

    def plot_return_curve(self, index, epoch_path):
        cmap1 = plt.cm.Blues  # 第一组色盘 (蓝色系，4个颜色)
        cmap2 = plt.cm.Reds

        colors_group1 = [cmap1(i) for i in np.linspace(0.4, 0.9, 4)]  # 蓝色组
        colors_group2 = [cmap2(i) for i in np.linspace(0.4, 0.9, 3)]  # 红色组
        ...
        return_curve = (
            (self.train_df["mark_price"] / self.train_df["mark_price"].iloc[0] - 1)
            * 100
        ).values[: len(index)]
        timestamp = self.train_df["timestamp"].values[: len(index)]
        ...
        drawn_labels = set()
        for i in range(len(return_curve) - 1):  # 画出每个点
            if f"label {index[i][0]}" not in drawn_labels:
                plt.plot(
                    [timestamp[i], timestamp[i + 1]],  # x 坐标
                    [return_curve[i], return_curve[i + 1]],  # y 坐标
                    color=color_list[index[i][0]],  # 颜色取决于当前点的 label
                    linewidth=2,
                    label=f"label {index[i][0]}",
                )
                drawn_labels.add(f"label {index[i][0]}")
            else:
                plt.plot(
                    [timestamp[i], timestamp[i + 1]],  # x 坐标
                    [return_curve[i], return_curve[i + 1]],  # y 坐标
        ...
        # 按照 desired_order 对图例进行排序
        sorted_handles_labels = sorted(
            zip(handles, labels),
            key=lambda x: (
                desired_order.index(x[1]) if x[1] in desired_order else float("inf")
            ),
        )

        # 解压排序后的 handles 和 labels
        ...
        plt.savefig(
            os.path.join(epoch_path, "update_index_label.pdf"), bbox_inches="tight"
        )
