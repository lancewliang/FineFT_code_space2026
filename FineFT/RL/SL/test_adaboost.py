...
class trader(object):
    def __init__(self, args) -> None:
        ...
        self.model = joblib.load(
            os.path.join(args.model_save_path, self.dataset_name, "model.pkl")
        )
        # dataset
        ...
        self.test_path = os.path.join(args.model_save_path, self.dataset_name)

    def act_test(self, state, info):

        result = self.model.predict(state.reshape(1, -1))
        if result > 0:
            action = np.max(info["avaiable_action_list"])
        if result < 0:
            action = np.min(info["avaiable_action_list"])
        return action
