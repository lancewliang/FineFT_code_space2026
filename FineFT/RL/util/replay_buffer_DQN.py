...
class ReplayMemory:
    ...
    def reset(self):
        self.memory_counter = 0
        self.state_memory = torch.FloatTensor(self.memory_size, self.state_shape)
        self.action_memory = torch.LongTensor(self.memory_size)
        self.reward_memory = torch.FloatTensor(self.memory_size)
        self.done_memory = torch.FloatTensor(self.memory_size)
        self.state__memory = torch.FloatTensor(self.memory_size, self.state_shape)
    ...
    def sample(self, size):
        ...
        state_sample = torch.FloatTensor(size, self.state_shape).to(self.device)
        action_sample = torch.LongTensor(size, 1).to(self.device)
        reward_sample = torch.FloatTensor(size, 1).to(self.device)
        state__sample = torch.FloatTensor(size, self.state_shape).to(self.device)
...
class Multi_step_ReplayBuffer:
    ...
    def __init__(
        self, buffer_size, batch_size, device, seed, gamma, n_step, parallel_env=1
    ):
...
class Multi_step_ReplayBuffer_info:
    ...
    def __init__(
        ...
        self.experience = namedtuple(
            "Experience",
            field_names=["state", "action", "reward", "next_state", "done", "info"],
        )
    ...
    def sample(self):
        ...
        for key in self.info_key:
            infos[key] = (
                torch.from_numpy(
                    np.stack(
                        [e.info[key] for e in experiences if e is not None]
                    ).astype(np.uint8)
                )
                .float()
                .to(self.device)
...
class Multi_step_ReplayBuffer_multi_info:
    ...
    def calc_multistep_return(self, n_step_buffer):
        ...
        for i in reversed(range(self.n_step)):  # 逆序计算n_steps_reward
            ...
            n_steps_reward = (
                single_reward + self.gamma * (1 - single_terminal) * n_steps_reward
            )
            if (
                single_terminal
    ...
    def sample_evaluate(self):
        experiences = list(self.memory)  # Preserve order instead of random sampling
        ...
        dones = (
            torch.from_numpy(
                np.vstack([e.done for e in experiences if e is not None]).astype(float)
            )
            .float()
            .to(self.device)
        )
...
class Multi_step_ReplayBuffer_multi_info_sunrise:
    """Fixed-size buffer to store experience tuples."""
    ...
    def add(
        ...
        self.n_step_buffer[self.iter_].append(
            (mask, state, info, action, reward, next_state, next_info, done)
        )
        ...
        if len(self.n_step_buffer[self.iter_]) == self.n_step:
            ...
            e = self.experience(
                mask, state, info, action, reward, next_state, done, next_info
            )
            self.memory.append(e)
        self.iter_ += 1
    ...
    def calc_multistep_return(self, n_step_buffer):
        ...
        for i in reversed(range(self.n_step)):  # 逆序计算n_steps_reward
            ...
            if (
                ...
                n_steps_reward = 0
        self.info_key = n_step_buffer[0][6].keys()
    ...
    def sample(self):
        ...
        experiences = random.sample(self.memory, k=self.batch_size)
        mask = (
            torch.from_numpy(np.stack([e.mask for e in experiences if e is not None]))
            .float()
            .to(self.device)
        )
        states = (
        ...
        return (mask, states, infos, actions, rewards, next_states, next_infos, dones)
...
class Multi_step_ReplayBuffer_multi_info_qmix_rl:
    def __init__(self, buffer_size, batch_size, device, seed, gamma, parallel_env=1):
        self.buffer_size = buffer_size
        self.device = device
        self.memory = deque(maxlen=buffer_size)
        self.batch_size = batch_size
        self.info_key_useful = ["higher_bound", "lower_bound", "state_array"]
        self.experience = namedtuple(
            "Experience",
            field_names=[
                "policy_reward_list",
                ...
                "action",
                "reward",
                "next_policy_reward_list",
                "next_base_reward_list",
                "done",
                "next_info",
            ],
        )
    ...
    def add(
        self,
        policy_reward_list: list,
        base_reward_list: list,
        info: dict,
        action,
        reward,
        next_policy_reward_list: list,
        next_base_reward_list: list,
        next_info: dict,
        done,
    ):
        if self.iter_ == self.parallel_env:
    ...
    def sample(self):
        ...
        policy_rewards = (
            torch.from_numpy(
                np.stack([e.policy_reward_list for e in experiences if e is not None])
            )
            .float()
            .to(self.device)
        )
        base_rewards = (
            torch.from_numpy(
                np.stack([e.base_reward_list for e in experiences if e is not None])
            )
            .float()
            .to(self.device)
        )
        infos = dict()
        for key in self.info_key:
            if key in self.info_key_useful:
                infos[key] = (
                    torch.from_numpy(
                        np.stack(
                            [e.info[key] for e in experiences if e is not None]
                ...
                if len(infos[key].shape) == 1:
                    infos[key] = infos[key].unsqueeze(1)
        actions = (
            torch.from_numpy(
                np.vstack([e.action for e in experiences if e is not None])
            )
        ...
        policy_rewards_next = (
            torch.from_numpy(
                np.stack(
                    [e.next_policy_reward_list for e in experiences if e is not None]
                )
            )
            .float()
            .to(self.device)
        ...
        base_rewards_next = (
            torch.from_numpy(
                np.stack(
                    [e.next_base_reward_list for e in experiences if e is not None]
                )
            )
            .float()
            .to(self.device)
        ...
        for key in self.info_key:
            if key in self.info_key_useful:
                ...
                if len(next_infos[key].shape) == 1:
                    next_infos[key] = next_infos[key].unsqueeze(1)
        dones = (
            torch.from_numpy(
                np.vstack([e.done for e in experiences if e is not None]).astype(float)
            )
...
class Multi_step_Prioritized_ReplayBuffer_multi_info(object):
    ...
    def store_transition(
        ...
        if len(self.n_steps_deque) == self.n_steps:
            ...
            self.buffer["next_avaliable_action"][self.count] = next_avaliable_action
            ...
            self.sum_tree.update(
                data_index=self.count, priority=priority
    ...
    def get_n_steps_transition(self):
        (
            ...
            q_action,
            action,
        ) = self.n_steps_deque[0][
            :5
        ]  # 获取deque中第一个transition的s和a
        (
            next_market_state,
            next_previous_action,
            next_avaliable_action,
