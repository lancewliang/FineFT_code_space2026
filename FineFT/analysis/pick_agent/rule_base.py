...
class Picker:
    def __init__(self, args) -> None:
        self.algorithm_list = ["macd", "imbalace_volume"]
    ...
    def analysis_all_epoch_single_algorithm(self, algorithm_name):
        result_list = []
        model_root_path = "result/base/{}/{}".format(
            algorithm_name,
            self.dataset_name,
        )
        para_list = os.listdir(model_root_path)
        for para in para_list:
            epoch_path = os.path.join(model_root_path, para)
    ...
    def create_best_agent_single_algorithm(self, best_results_df, algorithm_name):
        ...
        with open(
            os.path.join(high_level_path, "para.txt"), "w", encoding="utf-8"
        ) as file:
            file.write(para)
