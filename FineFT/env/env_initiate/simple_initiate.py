...
# env design for simple baselines, which include traditional RL method and lstm based RL method
def initiate_simple_env(
    df: pd.DataFrame,
    feature_list: list,
...
def initiate_rolling_env(
    ...
    markprice_array = df["mark_price"].values[window_size-1:]
    timestamp_array = df["timestamp"].values[window_size-1:]
    funding_rate_array = df["funding_rate"].values[window_size-1:]
    funding_timestamp_array = df["funding_timestamp"].values[window_size-1:]
    ask_prices_array = df[ask_prices_names].values[window_size-1:]
    bid_prices_array = df[bid_prices_names].values[window_size-1:]
    ask_qtys_array = df[ask_sizes_names].values[window_size-1:]
    bid_qtys_array = df[bid_sizes_names].values[window_size-1:]
    
    state_array = df[feature_list].values

    state_array = sliding_window_view(
        state_array, window_shape=(window_size, state_array.shape[1])
    )
    state_array = state_array.squeeze(axis=1)
...
if __name__ == "__main__":
    ...
    s, info = env.reset()
    for i in range(100):
        a = random.choice(info["avaiable_action_list"])
